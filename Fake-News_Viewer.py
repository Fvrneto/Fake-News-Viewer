...
df_cleaned_reliable = df_cleaned.drop(df[df.label == 0].index)
df_cleaned_unreliable = df_cleaned.drop(df[df.label == 1].index)
...
df_token["text_joined"] = df_token["text"].apply(lambda x: " ".join(x))
reliable = " ".join(df_token[df_token["label"]==0]["text_joined"])
unreliable = " ".join(df_token[df_token["label"]==1]["text_joined"])
...
plt.axis("off")
plt.title("Reliable Text WordCloud")
...
plt.imshow(wordcloud)
plt.axis("off")
plt.title("Unreliable Text WordCloud")
plt.tight_layout(pad = 0)
plt.show()
plt.close()

# Wordcloud for user text
